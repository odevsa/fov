...
function drawCarImage(carType) {
  ...
  const x =
    (canvasWidth - imageWidth * scale) / 2 + carType.offset.horizontal.shift;
  const y =
    (canvasHeight - imageHeight * scale) / 2 + carType.offset.vertical.shift;
...
function drawMeasurements(
  ...
  const displayDistance = Math.round(
    unit == "inch" ? distance / INCH_TO_CM : distance,
  );
  ...
  if (tripleScreenAngle) {
    ...
    const tripleFovText = `Monitor Angle: ${tripleScreenAngle.toFixed(1)}°`;
    ctx.strokeText(tripleFovText, 30, DRAW.measurementFontSize);
    ctx.fillText(tripleFovText, 30, DRAW.measurementFontSize);
  } else {
    const fovText = `FOV: ${fov.toFixed(1)}°`;
    ctx.strokeText(fovText, 30, DRAW.measurementFontSize / 3);
    ctx.fillText(fovText, 30, DRAW.measurementFontSize / 3);
...
function drawCarBox(carPosition) {
  ...
  ctx.fillRect(
    carPosition.x,
    carPosition.topY - carPosition.width,
    carPosition.length,
    carPosition.width,
  );
  ctx.fillRect(
    carPosition.x,
    carPosition.bottomY - carPosition.height,
    carPosition.length,
    carPosition.height,
  );
}
